...
class Agent:
    ...
    def monte_move(self,model):
        '''random Monte Carlo step for a tempering process'''
        rand_direction= np.array((float(np.random.uniform(-1,1,1)),float(np.random.uniform(-1,1,1))))
 #       new_rand_loc=self.location + (np.random.randint(35, size=self.speed.shape) * rand_direction)
        new_rand_loc=self.location + (np.random.randint(70, size=self.speed.shape) * rand_direction)

        
        self.location = new_rand_loc
        
        
    def monte_step(self, model):
        ''' Take a random Monte Carlo step used for tempering
        '''
        if self.status == 0:
            self.activate()
        elif self.status == 1:
            self.monte_move(model)
            self.deactivate()
        self.history()
...
class Model:
    ...
    def step_mc(self):
        '''
        Agents are moved by a Monto Carlo process for use in the tempering process.
        '''

        [agent.monte_step(self) for agent in self.agents]
    ...
    def get_distace_plot(self, real_data_dir, frame_i, frame_f):
        self.graphX1 = []; self.graphY1 = []; self.graphERR1 = [] # x, y, dy
        data = []
        for frame in range(frame_i, frame_f):
            ID, x, y = np.loadtxt(real_data_dir + str(frame) + '.0.dat', unpack=True)
            dist = []
            for i in range(len(ID)):
                agent_ID = int(ID[i])
                r1 = self.agents[agent_ID].history_locations[frame]
                r2 = (x[i], y[i])
