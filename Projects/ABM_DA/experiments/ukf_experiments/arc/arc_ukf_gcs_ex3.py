...
def ex3_parameters(parameter_lists, test):
    ...
    Returns
    -------
    n, run_id, n_jumps : int
        Agent population `n` and unique `run_id` for each n and prop. Also
        `n_jumps` number of agents whose gates are redrawn for each
    prop : float
...
def arc_ex3_main(parameter_lists, test):
    ...
    file_name = "gcs_rjukf_agents_{}_jump_rate_{}-{}".format(
        str(n).zfill(3),
        str(jump_rate),
