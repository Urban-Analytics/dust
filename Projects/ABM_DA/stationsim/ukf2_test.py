...
class ukf:
    ...
    def unscented_Mean(self, pool, sigmas, wm, kf_function, function_kwargs):
        ...
        **function_kwargs : kwargs
        ...
        if kf_function.__name__ == "fx":
            pool_out = starmap_with_kwargs(pool, 
                                        kf_function, 
                                        sigmas_iter, 
                                        function_kwargs)
            nl_sigmas = [out[0] for out in pool_out]
            base_models = [out[1] for out in pool_out]
            nl_sigmas = np.vstack(nl_sigmas).T
            u_mean = np.dot(nl_sigmas, wm)  # unscented mean for predicitons
            
            return nl_sigmas, u_mean, base_models
            ...
            return nl_sigmas, u_mean
    
    def predict(self, ukf_step, pool, fx_kwargs):
        ...
        if self.sigmas is None:
            "no sigmas just after update. rebuild sigmas"
            "and assign them to base_models using unscented_mean "
            self.sigmas = MSSP(self.x, self.p, self.g)
            self.sigmas, xhat, base_models = self.unscented_Mean(pool, self.sigmas, 
                                                          self.wm, 
                                                          self.fx,
                                                          fx_kwargs)
            self.x = xhat  # update xhat
        
        elif self.sample_rate > 1 and ukf_step % self.sample_rate == 0:
            "just before update take unscented mean and sigmas ready for update"
            self.sigmas, xhat, base_models = self.unscented_Mean(pool, self.sigmas, 
                                                          self.wm, 
                                                          self.fx,
                                                          fx_kwargs)
            ...
            "otherwise just step the models forwards"
            base_models = [kwarg["base_model"] for kwarg in fx_kwargs]
            base_models = pool.map(model_step, 
                                   base_models)
        return base_models
    ...
    def update(self, z, pool, hx_kwargs):
        ...
        hx_kwargs : dict
        """
        pxx = covariance(self.sigmas, self.x, self.wc, addition=self.q)
...
class ukf_ss:
    ...
    def ss_Predict(self, ukf_step, pool):
        ...
        self.base_models = self.ukf.predict(ukf_step, pool, self.fx_kwargs)
