...
class Model:
    ...
    def state2agents(self, state, noise=0):
        '''
        ...
        for i in range(len(self.buses)):
            ...
            self.buses[i].position = state[4 * i + 1] + np.random.normal(0, noise)
    ...
    def mask(self):
        # Edit this for
        _mask = np.ones(self.agents2state(True).shape, 'b')
        return _mask, 1
...
if __name__ == '__main__':  # Let's run the model
    ...
    import sys
    sys.path.append('..')
    from awest.code.data_assimilation.ParticleFilter import ParticleFilter

    pf = ParticleFilter(model, particles=20, window=1, do_copies=True, do_save=True)
