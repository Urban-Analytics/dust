"""
This function calibrates the BusSim model for the case when \xi increases from 1 to 20%
...
def unpickle_initiation(IncreaseRate):
...
if __name__ == '__main__':  # Let's run the model
    ...
    for IncreaseRate in range(11,21,2):
        #load model parameters
        # Note that we need the Increase Rate variable 
        ...
        for ite in range(CEM_params['MaxIteration']):
            ...
            if abs((best_PI-np.mean(last5best))/best_PI) < 0.05 and abs((best_PI-last5best[-1])/best_PI) < 0.05: #break the loop if the best mean doesn't change more than 5% compared to the average best PI
                ...
                name1 = ['BusSim_Model2_calibration_IncreaseRate_',str(IncreaseRate),'.pkl']
                str2 = ''.join(name1)
