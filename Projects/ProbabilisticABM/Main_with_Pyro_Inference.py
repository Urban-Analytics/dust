...
def main():
	"""
	This main routine will run the agent using pyro's EmpiricalMarginal and Importance methods as opposed to the main.py
	which uses a custom loop to step the agent. This main routine runs but is less flexible when it comes to visualising
	due to the samples in the posterior being private. It was also found that trying to pass the posterior back as the
	prior did not yield clear results but did step the model forward in the expected direction.
