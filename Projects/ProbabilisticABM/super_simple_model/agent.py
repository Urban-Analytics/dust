...
class Agent:
	"""
	The agent class for this project represents a simple agent that has randomness embedded within it using probabilistic
	programming package, Pyro as well as some common functionality such as movement and  storing location, destination,
	destination bias, speed and sample size.
	"""
	...
	def step(self, posterior=None, obs=None):
		...
		initialised as x, y = (0, 500) by default. The function will also take in observations and condition the built in
		Pyro normal distribution sampler on this input. If given a posterior object such as one build by Empirical Marginal,
		it will it's mean and stddev attributes and use those as to shape the new xy position (Though this has not been
		fully tested and may yield strange results).
		:param posterior:
		:param obs:
		:return:
		"""
		self.xy = pyro.sample('xy', dist.Normal(loc=self.xy if posterior is None else posterior.mean[0],
		                                        scale=tensor([[1.], [1.]]) if posterior is None else posterior.stddev[0]),
		                      obs=obs)
		self.xy = self.move(origin=self.xy, destination=self.destination)

	def move(self, origin, destination):
		"""
		The Lerp function(adapted from Kieran's and Andrew's work) will calculate the line needed to get a specified destination and steps along this line using
		a static random variable velocity initialise when the agent is built.
		:param origin:
		:param destination:
		:return: loc
		"""
